...
fn main() {
    ...
    let swift_triple = match target_arch.as_str() {
        ...
        other => panic!(
            "cgevents: unsupported target arch '{other}'. Expected x86_64 or aarch64."
        ),
    };
    ...
    println!("cargo:rustc-link-lib=static=CGEventsBridge");
    ...
    match Command::new("xcode-select").arg("-p").output() {
        Ok(output) if output.status.success() => {
            ...
            println!("cargo:rustc-link-lib=static=swiftCompatibilityDynamicReplacements");
